    ...
    <div class="row h-100 g-0 position-relative">
        ...
        <div class="col-lg-4 cart-sidebar" id="cartSidebar">
            <div class="cart-header">
                <div>
                    <h5 class="fw-extrabold text-slate-800 mb-0">Editar Traslado</h5>
                    <span class="text-muted small">Traslado #{{ str_pad($transfer->nro_traslado, 6, '0', STR_PAD_LEFT) }}</span>
                </div>
                ...
                <div class="mb-0">
                    ...
                    <select id="destination_branch_id" class="form-select border-0 shadow-sm rounded-4 py-2 px-3">
                        ...
                        @foreach($branches as $branch)
                            <option value="{{ $branch->id }}" {{ $transfer->destination_branch_id == $branch->id ? 'selected' : '' }}>{{ $branch->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="checkout-footer">
                <div class="mb-4">
                    ...
                    <textarea id="notes" class="form-control border-0 bg-slate-100 rounded-4 p-3" rows="3" placeholder="Motivo del traslado, conductor, etc..." style="background: #f1f5f9;">{{ $transfer->notes }}</textarea>
                </div>
                ...
                <button id="btnSaveTransfer" class="btn-pay-action d-flex align-items-center justify-content-center" disabled>
                    <i class="fas fa-sync-alt me-2"></i> ACTUALIZAR ENVÍO
                </button>
            </div>
        </div>
    </div>
